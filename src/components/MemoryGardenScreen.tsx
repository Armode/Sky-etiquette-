  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto space-y-8 pr-4 scrollbar-thin scrollbar-thumb-white/10">
        <div className="text-white/60 italic font-serif text-lg leading-relaxed max-w-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {state.memories.map((memory) => (
            ...
            >
              <div className="text-orange-300/50 text-xs uppercase tracking-widest mb-3">
                {new Date(memory.timestamp).toLocaleDateString()}
              </div>
              <p className="text-white/80 font-serif leading-relaxed group-hover:text-white transition-colors">
                {memory.text}
              </p>
            </motion.div>
          ))}
